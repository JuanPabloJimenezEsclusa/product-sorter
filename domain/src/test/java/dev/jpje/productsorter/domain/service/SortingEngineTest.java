...
import static dev.jpje.productsorter.domain.vo.CriterionType.STOCK_RATIO;
...
import dev.jpje.productsorter.domain.service.SortingEngine.ScoredEntry;
...
class SortingEngineTest {
  ...
  void shouldSortProductsAsSpecifiedInDocument(final List<ScoreableProduct> products,
    ...
    final var ids = sorted.stream().map(s -> s.productId().value()).toList();
