...
for entry in "${sessions[@]}"; do
  ...
  echo "[4/6] App warmup (${warmup}s, ${warmup_vus} VUs) ..."
  cd "${PROJECT_DIR}"
  ...
  echo "[5/6] Testing 10→50→100 users (steady=${STEADY_DURATION:-90s}, ramp=${RAMP_DURATION:-30s})..."
  docker compose -f docker-compose.yml -f perf/docker-compose.perf.yml --profile perf run --rm \
    -e APP_URL=http://product-sorter-app:8880 \
    -e KEYCLOAK_URL=http://keycloak:8080 \
